...
export const TodoCard: React.FC<TodoCardProps> = ({
  ...
  const handleToggleChecklistItem = async (itemId: string, currentStatus: boolean) => {
    try {
      ...
      // We need to get the updated todo to check the checklist status
      const updatedTodo = await TodoService.getTodoById(todo.id);
      if (updatedTodo && updatedTodo.checklist && updatedTodo.checklist.length > 0) {
        const allCompleted = updatedTodo.checklist.every(item => item.completed);
        if (allCompleted && updatedTodo.status !== 'completed') {
          // Auto-complete the todo if all checklist items are done
          ...
          <View style={styles.titleContainer}>
            ...
            {todo.deadline && (
              <View style={[
                ...
                {new Date(todo.deadline) < new Date() && todo.status !== 'completed' && (
