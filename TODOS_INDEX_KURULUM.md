# Todos Index Kurulum Rehberi

Project Details > To-Do sayfasında Firebase index hatası alınıyor. Firebase Firestore'da composite index gerekiyor.
...
✅ `firestore.indexes.json` dosyasına todos index'i zaten eklenmiş (satır 84-95)
...
1. **Index'in oluşturulduğundan emin olun:**
   ...
   - `todos` collection'ında index'in "Enabled" olduğunu kontrol edin
...
`todoService.ts` dosyasında fallback mekanizması var. Index yoksa client-side sorting kullanılıyor, ancak real-time subscription için index gerekiyor. Index oluşturulduktan sonra real-time güncellemeler düzgün çalışacaktır.
