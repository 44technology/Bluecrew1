...
type WorkTitleForm = { name: string; descriptions: WorkDescriptionItem[]; price: string };
...
export default function ProposalsScreen() {
  ...
  const totalCost = useMemo(() => {
    ...
    return total;
  }, [workTitles, newWorkTitle.descriptions, newProposal.supervision_weeks, newProposal.supervision_type, newProposal.general_conditions_percentage, newProposal.discount]);
  ...
  const handleAddWorkTitle = () => {
    ...
    setWorkTitles(prev => [...prev, { name: newWorkTitle.name.trim(), descriptions: [...newWorkTitle.descriptions], price: total.toString() }]);
  ...
  const handleUpdateWorkTitle = () => {
    ...
    setWorkTitles(prev => prev.map((wt, i) =>
      i === editingWorkTitleIndex ? { name: newWorkTitle.name.trim(), descriptions: [...newWorkTitle.descriptions], price: total.toString() } : wt
    ));
    setNewWorkTitle({ name: '', descriptions: [], price: '' });
  ...
  const handleCreateProposal = async () => {
    ...
    if (newWorkTitle.name.trim()) {
      const total = workTitlePrice(newWorkTitle);
      finalWorkTitles = [...finalWorkTitles, { name: newWorkTitle.name.trim(), descriptions: [...newWorkTitle.descriptions], price: total.toString() }];
    }
    ...
    if (!newProposal.client_street || !newProposal.client_street.trim()) {
      ...
      errors.client_street = 'Street Address is required';
    }
    ...
    if (!newProposal.client_zip || !newProposal.client_zip.trim()) {
      ...
      errors.client_zip = 'ZIP Code is required';
    }
    ...
    try {
      ...
      if (editingProposal) {
        const supervisionWeeks = parseFloat(newProposal.supervision_weeks) || 0;
        const supervisionType = newProposal.supervision_type;
        
        // If supervision_type is 'none', fee should be 0
        if (supervisionType === 'none') {
          supervisionFee = 0;
        }
        // If supervision_weeks is empty/0 (user didn't provide it), preserve original fee
        else if (supervisionWeeks === 0 || !newProposal.supervision_weeks.trim()) {
          supervisionFee = editingProposal.supervision_fee || 0;
        } else {
          // Calculate new fee if both type and weeks are provided
          const supervisionRate = supervisionType === 'full-time' ? 1450 : supervisionType === 'part-time' ? 725 : 0;
          supervisionFee = supervisionRate * supervisionWeeks;
        }
      } else {
        // For new proposals, calculate normally
        const supervisionWeeks = parseFloat(newProposal.supervision_weeks) || 0;
        const supervisionRate = newProposal.supervision_type === 'full-time' ? 1450 : newProposal.supervision_type === 'part-time' ? 725 : 0;
        supervisionFee = (supervisionWeeks > 0 && newProposal.supervision_type !== 'none') ? supervisionRate * supervisionWeeks : 0;
      }
      const generalConditionsPercentageInput = newProposal.general_conditions_percentage.trim();
      const generalConditionsPercentage = generalConditionsPercentageInput === '' ? 18.5 : (isNaN(parseFloat(generalConditionsPercentageInput)) ? 18.5 : parseFloat(generalConditionsPercentageInput));
      ...
      setNewDescriptionUnitPrice('');
            ...
            <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.inputGroup}>
                ...
                <TouchableOpacity
                  style={[styles.clientDropdownButton, fieldErrors.client_name && styles.inputError]}
                  onPress={() => {
                    setShowClientDropdown(!showClientDropdown);
                    if (fieldErrors.client_name) {
                      setFieldErrors(prev => ({ ...prev, client_name: '' }));
                    }
                  }}
                >
                  <Text style={[
                ...
                {fieldErrors.client_name && (
                  <Text style={styles.errorText}>{fieldErrors.client_name}</Text>
                )}
                {showClientDropdown && (
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => {
                    ...
                    if (fieldErrors.client_street) {
                      setFieldErrors(prev => ({ ...prev, client_street: '' }));
                    }
                  }}
                />
                {fieldErrors.client_street && (
                  <Text style={styles.errorText}>{fieldErrors.client_street}</Text>
                )}
              </View>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) => {
                    ...
                    if (fieldErrors.client_state) {
                      setFieldErrors(prev => ({ ...prev, client_state: '' }));
                    }
                  }}
                />
                {fieldErrors.client_state && (
                  <Text style={styles.errorText}>{fieldErrors.client_state}</Text>
                )}
              </View>
              ...
              <View style={styles.inputGroup}>
                ...
                <TouchableOpacity
                  style={[styles.categorySelectButton, fieldErrors.category && styles.inputError]}
                  onPress={() => {
                                ...
                                <View key={descIndex} style={{ marginBottom: 4 }}>
                                  ...
                                  {typeof desc !== 'string' && ((desc as WorkDescriptionItem).quantity || (desc as WorkDescriptionItem).unit_price) && (
                                    <Text style={[styles.workTitleDetailText, { marginLeft: 12, fontSize: 12 }]}>
                                      Qty: {(desc as WorkDescriptionItem).quantity || '0'} × ${(parseFloat((desc as WorkDescriptionItem).unit_price || '0')).toLocaleString(undefined, { minimumFractionDigits: 2 })} = ${(parseFloat((desc as WorkDescriptionItem).quantity || '0') * parseFloat((desc as WorkDescriptionItem).unit_price || '0')).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                    </Text>
                                  )}
                                </View>
                              ))}
                ...
                <View style={styles.addWorkTitleForm}>
                  ...
                  {(newWorkTitle.descriptions.length > 0 && descTotal(newWorkTitle.descriptions) > 0) && (
                    <View style={styles.calculatedPriceContainer}>
                      <Text style={styles.calculatedPriceLabel}>Total:</Text>
