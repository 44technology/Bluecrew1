...
export default function MaterialRequestScreen() {
  ...
  const handleAddRequest = async () => {
    ...
    if (!newRequest.quantity || !newRequest.quantity.trim()) {
      errors.quantity = 'Quantity is required';
    }
    if (!newRequest.description || !newRequest.description.trim()) {
      errors.description = 'Description is required';
    }
    if (!newRequest.delivery_date) {
      errors.delivery_date = 'Delivery Date is required';
    }
    if (!newRequest.vendor_id) {
  ...
  const handleRequestChanges = async (requestId: string) => {
    ...
    try {
      await MaterialRequestService.updateMaterialRequest(requestId, {
        ...
        change_request_by_name: user?.name || 'Admin',
  ...
  const handleUpdateRequest = async () => {
    ...
    if (!editRequest.quantity || !editRequest.quantity.trim()) {
      errors.quantity = 'Quantity is required';
    }
    if (!editRequest.description || !editRequest.description.trim()) {
      errors.description = 'Description is required';
    }
    if (!editRequest.delivery_date) {
      errors.delivery_date = 'Delivery Date is required';
    }
    if (!editRequest.vendor_id) {
      errors.vendor_id = 'Vendor is required';
    }
    ...
    try {
      await MaterialRequestService.updateMaterialRequest(editingRequest.id, {
        quantity: editRequest.quantity,
        description: editRequest.description,
        delivery_date: editRequest.delivery_date,
        vendor_id: editRequest.vendor_id,
        status: 'pending', // Reset to pending after update
        change_request_at: null,
        change_request_by: null,
        change_request_by_name: null,
      ...
      Alert.alert('Error', 'Failed to update material request');
    }
  };
        ...
        {requests.length === 0 ? (
          ...
          <View key={request.id} style={styles.requestCard}>
            ...
            <View style={styles.cardActions}>
              {/* Only allow editing if status is pending and no change request has been submitted */}
              {(userRole === 'sales' || userRole === 'pm') && request.status === 'pending' && !request.change_request_at && (
                <>
                  <TouchableOpacity
              ...
              {userRole === 'admin' && request.status === 'pending' && !request.change_request_at && (
                <TouchableOpacity
                  style={[styles.actionButton, styles.changeRequestButton]}
                  onPress={() => {
                    setSelectedRequest(request);
                    setShowChangeRequestModal(true);
                  }}>
                  <Edit size={16} color="#ffffff" />
                  <Text style={styles.changeRequestButtonText}>Request Changes</Text>
                </TouchableOpacity>
              )}
              
              {/* Show change request info if it exists */}
              {request.change_request_at && (
                <View style={styles.changeRequestInfo}>
                  <Text style={styles.changeRequestInfoText}>
                    Change requested by {request.change_request_by_name || 'Admin'} on {new Date(request.change_request_at).toLocaleDateString()}
                  </Text>
                  {request.change_request_reason && (
                    <Text style={styles.changeRequestReasonText}>
                      Reason: {request.change_request_reason}
                    </Text>
                  )}
                </View>
              )}
          ...
          <ScrollView style={styles.modalContent}>
            <View style={styles.formGroup}>
                ...
                {projects.map((project) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      fieldErrors.project_id && !newRequest.project_id && styles.inputError
                    ]}
                    onPress={() => {
                      setNewRequest(prev => ({ ...prev, project_id: project.id }));
              ...
              {fieldErrors.project_id && (
                <Text style={styles.errorText}>{fieldErrors.project_id}</Text>
              )}
            </View>
              ...
              <TextInput
                ...
                onChangeText={(text) => {
                  ...
                  const filteredValue = parts.length > 2 
                    ...
                    : numericValue;
                  setNewRequest(prev => ({ ...prev, quantity: filteredValue }));
            ...
            <View style={styles.formGroup}>
              ...
              {Platform.OS === 'web' ? (
                <input
                  ...
                  onChange={(e) => {
                    setNewRequest(prev => ({ ...prev, delivery_date: e.target.value }));
                    if (fieldErrors.delivery_date) {
                      setFieldErrors(prev => ({ ...prev, delivery_date: '' }));
                    }
                  }}
                  style={fieldErrors.delivery_date ? { ...styles.webDateInput, border: '2px solid #ef4444' } : styles.webDateInput}
                />
              ) : (
                <TouchableOpacity
                  style={[styles.datePickerButton, fieldErrors.delivery_date && styles.inputError]}
                  onPress={() => {
              ...
              {fieldErrors.delivery_date && (
                <Text style={styles.errorText}>{fieldErrors.delivery_date}</Text>
              )}
            </View>
            ...
            <ScrollView style={styles.dropdownList}>
              {vendors.map((vendor) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (showEditModal) {
                      setEditRequest(prev => ({ ...prev, vendor_id: vendor.id }));
                    } else {
            ...
            <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
              <View style={styles.detailSection}>
                ...
                <View style={styles.detailInfo}>
                  ...
                  {selectedRequest.change_request_at && (
                    <>
...
const styles = StyleSheet.create({
  ...
  changeRequestInfoText: {
    fontSize: 12,
    color: '#92400e',
    fontWeight: '600',
    marginBottom: 4,
  },
  changeRequestReasonText: {
    fontSize: 12,
    color: '#78350f',
  },
  updateButton: {
