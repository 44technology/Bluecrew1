...
export default function ProjectDetailScreen() {
  ...
  useLayoutEffect(() => {
    try {
      ...
      console.error('Error setting navigation options:', e);
    }
  }, [navigation]);
  ...
  const todoListRef = useRef<{ openCreateModal: () => void }>(null);
  ...
  const projectRate = project?.gross_profit_rate ?? 29;
  const internalBudgetFromSteps = project?.steps
    ?.filter(step => step.step_type === 'parent' && step.price != null)
    ?.reduce((sum, step) => {
      const rate = step.profit_rate ?? projectRate;
      return sum + ((step.price || 0) * (100 - rate)) / 100;
    }, 0) ?? 0;
  const hasStepTotals = stepBudgetsTotal > 0;
          ...
          <View style={styles.statusButtons}>
            {['pending', 'in_progress', 'finished'].map((status) => {
              const getStatusStyle = () => {
                if (userRole === 'client') {
                  // For clients, show colored buttons based on step status
                  if (step.status === status) {
                    switch (status) {
                      case 'pending': 
                        return [styles.clientStatusButton, styles.clientPendingButton];
                      case 'in_progress': 
                        return [styles.clientStatusButton, styles.clientInProgressButton];
                      case 'finished': 
                        return [styles.clientStatusButton, styles.clientFinishedButton];
                      default: 
                        return styles.clientStatusButton;
                    }
                  }
                  return styles.clientStatusButton;
                }
              ...
              const getStatusTextStyle = () => {
                if (userRole === 'client') {
                  // For clients, show colored text based on step status
                  if (step.status === status) {
                    switch (status) {
                      case 'pending': 
                        return [styles.clientStatusButtonText, styles.clientPendingButtonText];
                      case 'in_progress': 
                        return [styles.clientStatusButtonText, styles.clientInProgressButtonText];
                      case 'finished': 
                        return [styles.clientStatusButtonText, styles.clientFinishedButtonText];
                      default: 
                        return styles.clientStatusButtonText;
                    }
                  }
                  return styles.clientStatusButtonText;
                }
          ...
          <View style={styles.projectHeader}>
                    ...
                    ? (() => {
                        try {
                          ...
                          const pmNames = project.assigned_pms
                            .map((pmId: string) => {
                              if (!pms || !Array.isArray(pms)) return pmId;
                              const pm = pms.find(p => p && p.id === pmId);
                          ...
                          console.error('Error getting PM names:', e);
                          return project.assigned_pms[0] || 'N/A';
                        }
                      })()
            ...
            {(userRole === 'pm' || userRole === 'admin' || userRole === 'office' || userRole === 'client') && (
              <View style={styles.budgetSection}>
                {userRole === 'client' ? (
                  // Client sees total price = Step total + GC (when steps exist), else proposal/client_budget
                  <>
                    <Text style={styles.budgetLabel}>Total Price:</Text>
                    <Text style={styles.budgetAmount}>
                      ${(hasStepTotals ? clientTotalFromSteps : (project?.client_budget || project?.total_budget || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Text>
                  </>
                ) : (
                ...
                {userRole === 'pm' && project?.pm_budgets && user?.id && project.pm_budgets[user.id] ? (
                  ...
                  <>
                        ...
                        <View style={[styles.stepBudgetBreakdown, { backgroundColor: '#ecfdf5', padding: 10, borderRadius: 8, marginTop: 6, marginBottom: 8 }]}>
                          ...
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                            ...
                            <Text style={[styles.stepBudgetAmount, { color: '#059669', fontWeight: '600' }]}>
                              ${profitMarginFromSteps.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </Text>
                          </View>
                    ...
                    {(userRole === 'admin' || userRole === 'office') && !hasStepTotals && project?.client_budget == null && (project?.general_conditions != null && project.general_conditions > 0) && (
                    ...
                    <Text style={styles.budgetAmount}>
                      ${totalWithChanges.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  clientInProgressButtonText: {
    color: '#ea580c',
  },
  clientFinishedButton: {
    backgroundColor: '#d1fae5',
    borderColor: '#22c55e',
  },
  clientFinishedButtonText: {
    color: '#16a34a',
  },
  stepDescription: {
    fontSize: 14,
