...
import { Invoice, Comment, Proposal, ExpensePayment, ExpenseDocument, Project } from '@/types';
...
export default function InvoicesScreen() {
  ...
  useEffect(() => {
    ...
    // load related proposal and project info when invoice selected
    const fetchRelated = async () => {
      try {
        if (selectedInvoice?.proposal_id) {
          const proposal = await ProposalService.getProposalById(selectedInvoice.proposal_id);
          setRelatedProposal(proposal || null);
        } else {
          setRelatedProposal(null);
        }
      } catch (e) {
        console.error('Error loading related proposal:', e);
        setRelatedProposal(null);
      }

      try {
        if (selectedInvoice?.project_id) {
          const project = await ProjectService.getProjectById(selectedInvoice.project_id);
          setRelatedProject(project || null);
        } else {
          setRelatedProject(null);
        }
      } catch (e) {
        console.error('Error loading related project:', e);
        setRelatedProject(null);
      }
    };
  ...
  const handleExportPDF = async (invoice: Invoice) => {
    ...
    try {
      ...
      if (invoice.proposal_id) {
        try {
          const proposal = await ProposalService.getProposalById(invoice.proposal_id);
          if (proposal?.proposal_number) {
            proposalInfo = proposal.proposal_number;
          }
        } catch (error) {
          console.error('Error loading related proposal:', error);
        }
      }
      
      if (invoice.project_id) {
        try {
          const project = await ProjectService.getProjectById(invoice.project_id);
          if (project?.title) {
      ...
      // If project info not found from project_id, use project_name from invoice
      if (!projectInfo && invoice.project_name) {
        ...
        <body>
          ...
          <div class="info-section">
            ...
            <div class="info-box">
              ...
              ${proposalInfo ? `<div class="info-label" style="margin-top: 10px;">Related Proposal</div><div class="info-value" style="font-size: 14px;">${proposalInfo}</div>` : ''}
              ${projectInfo ? `<div class="info-label" style="margin-top: 10px;">Related Project</div><div class="info-value" style="font-size: 14px;">${projectInfo}</div>` : ''}
            </div>
          </div>
              ...
              {selectedInvoice && (
                <>
                  <View style={styles.detailSection}>
                    ...
                    {selectedInvoice.proposal_id && (
                      <View style={styles.detailRow}>
                        <Text style={styles.detailLabel}>Proposal:</Text>
                        <Text style={styles.detailValue}>
                          {relatedProposal?.proposal_number || 'N/A'}
                        </Text>
                      </View>
                    )}
                    <View style={styles.detailRow}>
